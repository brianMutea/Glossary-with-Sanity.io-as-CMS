...
export function TypeInput(props: StringInputProps) {
    ...
    const fetchSuggestions = useCallback(async () => {
        setLoading(true)
        try {
            const query = `*[_type == "glossaryTerm" && defined(type)] { type }`
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--card-muted-fg-color, #999)' }}>
                Start typing to see suggestions from existing types, or type a new value.
                Similar values will be automatically normalized.
